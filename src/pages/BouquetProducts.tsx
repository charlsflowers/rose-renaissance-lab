            ...
            {filteredProducts.map((product) => (
              <div key={product.id}>
                <Link to={`/bouquets/all/${product.id}`} className="group block">
                  ...
                  <p className="text-primary font-body text-sm font-semibold text-center mt-2">Desde ${product.customSizes ? product.customSizes[0].price : getPrice(product.pricingTier, product.pricingTier === 'mix3red' ? 75 : 50)}</p>
                </Link>
              </div>
            ))}
          </div>
